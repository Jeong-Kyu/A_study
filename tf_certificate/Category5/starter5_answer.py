...
def windowed_dataset(series, window_size, batch_size, shuffle_buffer):
    series = tf.expand_dims(series, axis=-1) # 마지막 차원추가
    ds = tf.data.Dataset.from_tensor_slices(series) # 각 항목 슬라이싱
    ds = ds.window(window_size + 1, shift=1, drop_remainder=True) # 그룹화할 개수, shift : 1 iteration당 몇 개씩 이동할 것인지, drop_remainder: 남은 부분을 버릴지 살릴지 여부
    ds = ds.flat_map(lambda w: w.batch(window_size + 1)) # 펼친다
    ds = ds.shuffle(shuffle_buffer) # 몇개의 데이터를 서플할것인가
    ds = ds.map(lambda w: (w[:-1], w[1:])) # 여러개의 데이터를 한번에 변경
