...
model = Sequential()
# model.add(Embedding(input_dim=28, output_dim=11, input_length=5)) # 벡터화 하여 거리를 계산함(불필요 데이터 압축) #input_dim=단어수, output_dim=다음레이어, input_length=컬럼수
model.add(Embedding(28,11)) # input_dim 은 단어수와 크거나 같아야한다.
model.add(LSTM(32))
