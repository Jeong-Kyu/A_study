...
from tensorflow.keras.layers import Dense, Input, Conv1D,MaxPool1D, Flatten
...
#2. 모델링
model  = Sequential()
model.add(Conv1D(filters = 10,kernel_size=10, input_shape = (10,1)))  # (input_dim * kernel_size + bias)*filter
#strides = 얼마나 건너서 자를건지 2 / (2,3)
model.add(MaxPool1D(pool_size=1))  # 2 / 3 / (2,3)
...
model.fit(x_train, y_train, epochs=100, batch_size=7, validation_data= (x_val, y_val), callbacks = [early_stopping])
...
# conv1d
# loss :  3159.207763671875
# mae :  46.495155334472656